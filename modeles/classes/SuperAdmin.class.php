<?php
	...
	class SuperAdmin extends Personne implements Connexion
	{
		...
		public  function reinitialiser($ancienMotDepasse, $nouveauModePasse){
			...
			$resultat = $this->check($this->getLogin(),$ancienMotDepasse, 'superadmin');
			if(empty($resultat)){
		...
		public function creerPharmacie($nom, $localisation,$photo, $nomD, $prenomD, $telD, $adresseD, $loginD, $passwordD){
			...
			if(!empty($SuperAdmin)){
				...
				if($valeur){
					...
					$reponse2 = $bd->prepare('SELECT * FROM directeur WHERE login = ? AND motDePasse = PASSWORD(?)');
		...
		function suspendrePharmacie($nom){
			...
			try {
				$dateJour = new \DateTime('now');
				$date = $dateJour->format('y-m-d H:i:s');
				$reponse = $bd->prepare('UPDATE pharmacie SET etat = ?, modifiedtAt = ? WHERE nom = ?');
